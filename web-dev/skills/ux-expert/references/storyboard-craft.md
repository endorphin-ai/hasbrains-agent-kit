...
> **Source**: adapted from [tommyjepsen/awesome-ux-skills](https://github.com/tommyjepsen/awesome-ux-skills) (`ux-storyboard.md`).
> **Load when**: Producing a storyboard — the 3 required components, the fidelity/audience table, the panel format, storyboard-vs-journey-map, the quality checklist.
...
- The source's `show_widget` visualizer tool does **not** exist in this environment. The default (and
  usually correct) output here is the **text/ASCII panel block** shown above, written into
  `docs/ux/storyboards/<scenario-slug>.md` — cheap to iterate, reviewable in a PR diff, and it lives in
  the committed system of record. If a rendered visual is genuinely wanted, hand the panel spec to
  the design owner; this skill does not author visual artifacts.
- Emotion markers stay as emoji in the markdown — they survive a `git diff` and are the point of the
  artifact.
- Give the **access-state seam** its own panel whenever the scenario crosses one (free → gated,
  anonymous → registered, entitled → expired). The emotional low reliably lives there, and it is the
  panel the team needs to see.
