...
per run; the orchestrator `init`s it at session start and passes its PATH as `state_file`). This note documents
...
```json
{
  ...
  "backend_code": ".ai_log/phase-3-backend-manifest.md",
  "view_code": ".ai_log/phase-4-frontend-manifest.md",
  ...
  "phases": {
    "0-requirements": {
      ...
      "evidence":   [".ai_log/phase-0-pm-prd.md"],
      "work_items": ["docs/epics/member-blog-gating.md", "docs/prd/member-blog-gating.md"]
    },
    "5-verification": {
      "agent": "architect",
      "status": "done",
      "outputs":    { "gate": "pass", "verification_report": ".ai_log/phase-5-architect-verification-report.md" },
...
| **Evidence-path list** (`.ai_log/` artifacts, screenshots, logs) | `append <session> <field> <path>` | `append S phase-6-evidence .ai_log/phase-6-qa-coverage.json` |
| **Nested `phases.<n-name>` entry** | the orchestrator writes it with `jq` directly (the script's `set`/`append` operate on flat top-level keys) | see the top-level shape above |

- A **per-phase entry** (`phases.<phase-number>-<phase-name>`) holds: `agent`, `status`
  (`todo -> in_progress -> done`, mirroring the project's work-item status lifecycle), `outputs` (each contract field ->
...
- **Never store a large blob** (full code dumps, full reports) as a value — store a PATH to the
  `.ai_log/` (evidence) or `docs/` (durable) artifact.
- The output-field NAMES match the run's declared contract keys (e.g. a workflow definition's `context_schema`)
...
2. **On COMPLETION:** `set`/`append` this phase's outputs — key result fields + PATHS to evidence
   ...
   evidence, work_items). Keep writing the docs/-native report as the human record; the state is the
   machine handoff bus and REFERENCES those docs/ + `.ai_log/` artifacts.

## Coexistence (do not conflate)

| Layer | Home | Purpose |
|-------|------|---------|
| **pipeline-state** | `.ai_log/session-<id>-<name>.json` (per-session, git-ignored) | MACHINE handoff bus — key fields + reference PATHS across phases |
| **`.ai_log/`** | `.ai_log/<phase-N>-<agent>-<artifact>.<ext>` | TEMPORARY evidence blobs, git-ignored |
| **docs/ reports** | `docs/sessions/<session>/`, `docs/reports/feature-log/` | HUMAN-readable committed work record |
